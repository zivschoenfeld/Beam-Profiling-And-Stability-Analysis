...
camera_sensor_size = float(input_txt_lines[1]) * 1000.0  # converting from mm to micrometer
input_txt.close()
...
def gauss(x,A,x0,sigma_x,C): 
    ...
    -----------
    '''
    I = A * np.exp(-((x - x0) ** 2) / (2 * sigma_x ** 2)) + C
    return I

# processing the first image to get pixel size and beam size and gaussian example plots:
img1 = tifffile.imread(f'{images_directory}\\{os.listdir(images_directory)[0]}')  # converting img to array
pixel_size = (camera_sensor_size / img1.shape[1])  # calculating pixel size in loop(micrometer)

# setting up the data
Ix_data = np.array([sum(x) for x in zip(*img1)])  # sum of each column of pixels in image as list, x
x = list(range(len(Ix_data)))

Iy_data = [sum(row) for row in img1]  # sum of each row of pixels in image as list, y
...
optimized_parameters_x, pcov_x = scipy.optimize.curve_fit(
    gauss, x, Ix_data,
    p0=[np.max(Ix_data), img1.shape[1] / 2, img1.shape[1] / 5, np.min(Ix_data)],
    bounds=((0, 0, 1, 0),
            (np.max(Ix_data) * 2, img1.shape[1], img1.shape[1] / 2, np.max(Ix_data)))
)

# y-axis:
optimized_parameters_y, pcov_y = scipy.optimize.curve_fit(
    gauss, y, Iy_data,
    p0=[np.max(Iy_data), img1.shape[0] / 2, img1.shape[0] / 5, np.min(Iy_data)],
    bounds=((0, 0, 1, 0),
...
plt.plot(np.linspace(x[0], x[-1], 100), gauss(np.linspace(x[0], x[-1], 100), *optimized_parameters_x), label="Gaussian fit")
...
plt.plot(np.linspace(y[0], y[-1], 100), gauss(np.linspace(y[0], y[-1], 100), *optimized_parameters_y), label="Gaussian fit")
...
beams_position_x = np.array([])
beams_position_y = np.array([])
...
for image_name in os.listdir(images_directory):
    ...
    img = tifffile.imread(f'{images_directory}\\{image_name}') #converting all images to arrays

   # setting up the data
    Ix_data = np.array([sum(x) for x in zip(*img)])  # sum of each column of pixels in image as list, x
    x = list(range(len(Ix_data)))

    Iy_data = [sum(row) for row in img]  # sum of each row of pixels in image as list, y
    y = list(range(len(Iy_data)))
   ...
   # x-axis:
    ...
    optimized_parameters_y, pcov_y = scipy.optimize.curve_fit(
        gauss, y, Iy_data,
        p0=[np.max(Iy_data), img.shape[0] / 2, img.shape[0] / 5, np.min(Iy_data)],
        bounds=((0, 0, 1, 0),
                (np.max(Iy_data) * 2, img.shape[0], img.shape[0] / 2, np.max(Iy_data)))
    )
    
    # saving beam positions
    beams_position_x = np.append(beams_position_x, optimized_parameters_x[1])
    beams_position_y = np.append(beams_position_y, optimized_parameters_y[1])
    # END OF MAIN LOOP
...
pointing_stability_x_axis = pixel_size * get_RMS(beams_position_x, average_beam_position_x) 
pointing_stability_y_axis = pixel_size * get_RMS(beams_position_y, average_beam_position_y)
radial_position_squared = (beams_position_x - average_beam_position_x)**2.0 + (beams_position_y - average_beam_position_y)**2.0
...
Hz_to_sec = 1.0/10.0
beams_position_time = list(range(len(beams_position_x*Hz_to_sec))) # time axis in seconds
plt.tight_layout()
